...
/**
 * An abstract implementation of the ListView Adapter which allows for the easy item tracking
...
public abstract class TrackableListViewAdapter<KT, IT extends BaseItem, VH extends BaseItem.ViewHolder<?>> extends BaseListViewAdapter<IT, VH> {
    ...
    public TrackableListViewAdapter(@NonNull Context context, @NonNull List<IT> items) {
        super(context, items);
    ...
    @Override
    public int getItemViewType(int position, IT item) {
        if(item == null) {
            return Item.VIEW_TYPE_INVALID;
        }
